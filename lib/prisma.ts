    ...
    case "otpVerification": {
      ...
      if (action === "deleteMany") {
        ...
        if (Array.isArray(cond)) {
          for (const c of cond) {
            if (c.email) mockOtps.delete(c.email);
            if (c.mobile) mockOtps.delete(c.mobile);
          }
        }
        return { count: 1 };
      }
      if (action === "create") {
        const item = { id: "otp-" + Date.now(), attempts: 0, ...arg.data };
        mockOtps.set(item.id, item);
        if (item.email) mockOtps.set(item.email, item);
        if (item.mobile) mockOtps.set(item.mobile, item);
        return item;
      }
      if (action === "findUnique" || action === "findFirst") {
        const id = arg?.where?.id;
        return mockOtps.get(id) || null;
      }
      if (action === "delete") {
        const id = arg?.where?.id;
        mockOtps.delete(id);
      ...
      if (action === "update") {
        const id = arg?.where?.id;
        const item = mockOtps.get(id) || { id, attempts: 0 };
        if (arg?.data?.attempts?.increment) item.attempts += arg.data.attempts.increment;
        mockOtps.set(id, item);
        return item;
      }
      break;
    }
